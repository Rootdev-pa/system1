                ...
                <table class="table-auto">
                    <thead>
                        <tr style="font-size: 0.9674rem">
                            ...
                            <th class="px-4 py-2 bg-gray-200" style="background-color:#f8f8f8">prueba</th>

                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="rol in list" :key="rol.id">
                            <td class="border px-4 py-2">
                                <span v-if="editmode ==false || editmode != rol.id">{{rol.slug}}</span>
                                ...
                                border-gray-200 rounded focus:outline-none focus:bg-white focus:border-gray-500" 
                                    ...
                                    v-model="rol.slug" type="text">
                            </td>

                            <td class="border px-4 py-2">
                                <span v-if="editmode ==false || editmode != rol.id">{{rol.descrip}}</span>
                                <input class="tracking-wide py-2 px-4 leading-relaxed appearance-none block w-full bg-gray-200 border
                                border-gray-200 rounded focus:outline-none focus:bg-white focus:border-gray-500" 
                                v-if="editmode == rol.id"
                                v-model="rol.descrip" type="text">
                            </td>
                            ...
                            <div>
                                <td class="border px-4 py-2">
                                    <div class="ml-auto mr-2 d-flex align-items-center">
                                        <span>
                                            <svg v-on:click="editmode = rol.id" v-if="editmode != rol.id"
